...
public class Wall_13 extends Wall{
	...
	protected void loadResources(){
		...
		this.mAnim = ((BaseActivity)this.context).createTR(this.mBitmapTexture, "painter_pink_anim.png", 44, 0);
		this.mPainter = ((BaseActivity)this.context).createTTR(this.mBitmapTexture, "painter_pink.png", 0, 44,16,1);
	...
	public int move(Player mPlayer){
		if (mPlayer.getPainter() != Constant.PINK_PAINTER) {
			mPlayer.setPainter(Constant.PINK_PAINTER);
			this.showAnim();
		}
		return Constant.PINK_PAINTER;
	};
	...
	private void showAnim(){
		...
		move.addModifierListener(new IModifierListener<IEntity>() {			
			...
			public void onModifierFinished(IModifier<IEntity> pModifier, IEntity pItem) {
				((BaseActivity)Wall_13.this.context).runOnUpdateThread(new Runnable(){
					@Override
					public void run() {
						anim.detachSelf();
					}				
