...
public class Wall_15 extends Wall{
	...
	protected void loadResources(){
		...
		this.mWall = ((BaseActivity)this.context).createTTR(this.mBitmapTexture, "wall_green_"+((GameActivity)this.context).getStage()+".png", 0, 0,2,1);
	...
	public int move(Player mPlayer){
		//刷墙
		if(mPlayer.getPainter() == Constant.GREEN_PAINTER && this.mWallSprite.getCurrentTileIndex()!=1){
	...
	private void showGetMoney(int pNum){		
		...
		move.addModifierListener(new IModifierListener<IEntity>() {			
			...
			public void onModifierFinished(IModifier<IEntity> pModifier, IEntity pItem) {
				((BaseActivity)Wall_15.this.context).runOnUpdateThread(new Runnable(){
					@Override
					public void run() {
