...
public  class Wall_11 extends Wall{
	...
	protected void loadResources(){
		...
		this.mAnim = ((BaseActivity)this.context).createTR(this.mBitmapTexture, "painter_green_anim.png", 44, 0);
		this.mPainter = ((BaseActivity)this.context).createTTR(this.mBitmapTexture, "painter_green.png", 0, 44,16,1);
	...
	public int move(Player mPlayer){
		if (mPlayer.getPainter() != Constant.GREEN_PAINTER) {
			mPlayer.setPainter(Constant.GREEN_PAINTER);
			this.showAnim();
		}
		return Constant.GREEN_PAINTER;
	};
	...
	private void showAnim(){
		...
		move.addModifierListener(new IModifierListener<IEntity>() {			
			...
			public void onModifierFinished(IModifier<IEntity> pModifier, IEntity pItem) {
				((BaseActivity)Wall_11.this.context).runOnUpdateThread(new Runnable(){
					@Override
					public void run() {
						anim.detachSelf();
					}				
