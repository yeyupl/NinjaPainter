...
public class Wall_12 extends Wall{
	...
	protected void loadResources(){
		...
		this.mAnim = ((BaseActivity)this.context).createTR(this.mBitmapTexture, "painter_blue_anim.png", 44, 0);
		this.mPainter = ((BaseActivity)this.context).createTTR(this.mBitmapTexture, "painter_blue.png", 0, 44,16,1);
	...
	public int move(Player mPlayer){
		if (mPlayer.getPainter() != Constant.BLUE_PAINTER) {
			mPlayer.setPainter(Constant.BLUE_PAINTER);
			this.showAnim();
		}
		return Constant.BLUE_PAINTER;
	};
	...
	private void showAnim(){
		...
		move.addModifierListener(new IModifierListener<IEntity>() {			
			...
			public void onModifierFinished(IModifier<IEntity> pModifier, IEntity pItem) {
				((BaseActivity)Wall_12.this.context).runOnUpdateThread(new Runnable(){
					@Override
					public void run() {
						anim.detachSelf();
					}				
