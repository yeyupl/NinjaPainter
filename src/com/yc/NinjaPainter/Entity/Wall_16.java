...
public class Wall_16 extends Wall{
	...
	protected void loadResources(){
		...
		this.mWall = ((BaseActivity)this.context).createTTR(this.mBitmapTexture, "wall_blue_"+((GameActivity)this.context).getStage()+".png", 0, 0,2,1);
	...
	public int move(Player mPlayer){
		//刷墙
		if(mPlayer.getPainter() == Constant.BLUE_PAINTER && this.mWallSprite.getCurrentTileIndex()!=1){
	...
	private void showGetMoney(int pNum){		
		...
		move.addModifierListener(new IModifierListener<IEntity>() {			
			...
			public void onModifierFinished(IModifier<IEntity> pModifier, IEntity pItem) {
				((BaseActivity)Wall_16.this.context).runOnUpdateThread(new Runnable(){
					@Override
					public void run() {
