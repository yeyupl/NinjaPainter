...
public class Wall_14 extends Wall{
	...
	protected void loadResources(){
		...
		this.mWall = ((BaseActivity)this.context).createTTR(this.mBitmapTexture, "wall_red_"+((GameActivity)this.context).getStage()+".png", 0, 0,2,1);
	...
	public int move(Player mPlayer){
		//刷墙
		if(mPlayer.getPainter() == Constant.RED_PAINTER && this.mWallSprite.getCurrentTileIndex()!=1){
			this.mWallSprite.setCurrentTileIndex(1);
			this.isPainted = true;
			final int score = 50+((GameActivity)this.context).getCount()*10;
	...
	private void showGetMoney(int pNum){		
		...
		move.addModifierListener(new IModifierListener<IEntity>() {			
			...
			public void onModifierFinished(IModifier<IEntity> pModifier, IEntity pItem) {
				((BaseActivity)Wall_14.this.context).runOnUpdateThread(new Runnable(){
					@Override
					public void run() {
